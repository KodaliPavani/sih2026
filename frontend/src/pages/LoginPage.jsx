...
import { GraduationCap, Building2, ShieldCheck, Lock, User, Sparkles, ArrowRight } from 'lucide-react';

export default function LoginPage() {
  const [activeTab, setActiveTab] = useState('STUDENT'); // 'STUDENT' or 'PLACEMENT_CELL'
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md z-10 px-4">
        <div className="glass-panel py-8 px-6 shadow-2xl rounded-2xl sm:px-10 border border-slate-800">
          ...
          <div className="flex p-1 bg-slate-900/80 rounded-xl border border-slate-800 mb-6">
            ...
            >
              <Building2 className="w-4 h-4" /> Placement Cell
            </button>
          </div>
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            <div>
              <label className="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-1.5">
                {activeTab === 'STUDENT' ? 'Student ID (Username)' : 'Admin ID'}
              </label>
              <div className="relative rounded-xl shadow-sm">
                ...
                <input
                  ...
                  placeholder={activeTab === 'STUDENT' ? 'e.g. 2300030042' : 'admin'}
          ...
          <div className="mt-6 pt-5 border-t border-slate-800">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              ...
              >
                <div className="font-bold text-sky-400">Student Demo</div>
                <div className="text-[11px] text-slate-400">2300030042</div>
              </button>
              <button
                type="button"
                onClick={() => {
                ...
                className="p-2 rounded-lg bg-slate-900 hover:bg-slate-800 border border-slate-800 text-left text-slate-300 hover:text-white transition-colors"
              >
                <div className="font-bold text-indigo-400">Placement Admin</div>
                <div className="text-[11px] text-slate-400">admin / placement123</div>
              </button>
            </div>
          </div>
